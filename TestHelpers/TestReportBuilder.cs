...
namespace DataImporter
{
    class TestReportBuilder
    {
        ...
        TestReportWriter reporter;

        public TestReportBuilder(string db_conn)
        {
            _db_conn = db_conn;

            // set up report file
            reporter = new TestReportWriter();

            IConfigurationRoot settings = new ConfigurationBuilder()
                .SetBasePath(AppContext.BaseDirectory)
                .AddJsonFile("appsettings.json")
                .Build();
            string logfile_startofpath = settings["logfilepath"];

            reporter.OpenLogFile(logfile_startofpath);
        }


        public int GetCount(string sql_string)
        {
            int res = 0;
        ...
        {
            reporter.LogHeader("Study record counts");

            sql_string = "select count(*) from expected.studies;";
            int e = GetCount(sql_string);
            reporter.LogLine("Number of expected studies: " + e.ToString());

            sql_string = "select count(*) from adcomp.studies;";
            int a = GetCount(sql_string);
            reporter.LogLine("Number of actual studies: " + a.ToString());

            sql_string = @"select count(*) from 
                         expected.studies e
                         inner join adcomp.studies a
                         on e.sd_sid = a.sd_sid;";
            int m = GetCount(sql_string);
            reporter.LogLine("Number of matched study ids: " + m.ToString());

            if (a != e)
            {
                reporter.LogError("Number of actual and expected studies do not match");
                reporter.LogError("Cannot proceed with testing studies until this error resolved");
                return false;
            }
            else if (a != m)
            {
                reporter.LogError("Number of matched study IDs not equal to total number");
                reporter.LogError("Cannot proceed with testing studies until this error resolved");
                return false;
            }
            else
            {
                reporter.LogLine("All OK!");
                reporter.LogLine("");
                TotalStudies = m;
                return true;
            }
        ...
        {
            reporter.LogHeader("Object record counts");

            sql_string = "select count(*) from expected.data_objects;";
            int e = GetCount(sql_string);
            reporter.LogLine("Number of expected data objects: " + e.ToString());

            sql_string = "select count(*) from adcomp.data_objects;";
            int a = GetCount(sql_string);
            reporter.LogLine("Number of actual data objects: " + a.ToString());
            ...
            int m = GetCount(sql_string);
            reporter.LogLine("Number of matched data objects: " + m.ToString());

            if (a != e)
            {
                reporter.LogError("Number of actual and expected data objects do not match");
                reporter.LogError("Cannot proceed with testing studies until this error resolved");
                return false;
            }
            else if (a != m)
            {
                reporter.LogError("Number of matched data object IDs not equal to total number");
                reporter.LogError("Cannot proceed with testing studies until this error resolved");
                return false;
            }
            else
        ...
        {
            reporter.LogHeader("Comparing " + field + " field in " + table + " table");
        }

        private void DoAttributeComparisonHeader(string table)
        {
            reporter.LogHeader("Comparing Attribute Numbers in " + table);
        }

        private void DoHashComparisonHeader(string table)
        {
            reporter.LogHeader("Comparing Hash values for " + table);
        }
        ...
        {
            int total = table == "studies" ? TotalStudies : TotalObjects;
            ...
            else
            {
                if (n > 0)
                {
                    ...
                    reporter.LogSimpleLine(list_header);
                    reporter.BlankLine();
        ...
        private int GetMatchingIds(string table, string field)
        {
            ...
            int m = GetCount(sql_string);
            reporter.LogLine("Number of matched field values: " + m.ToString());
            return m;
        }
        ...
        private int GetNonMatchingIds(string table, string field)
        {
            ...
            reporter.LogLine("Number of unmatched field values: " + n.ToString());
            return n;
        }
        ...
        private int GetBothNulls(string table, string field)
        {
            ...
            int b = GetCount(sql_string);
            reporter.LogLine("Number of records where both records null: " + b.ToString());
        ...
        private int GetNullsInExpectedIds(string table, string field)
        {
            ...
            int en = GetCount(sql_string);
            reporter.LogLine("Number of unmatched nulls in expected: " + en.ToString());
            return en;
        }
        ...
        private int GetNullsInActualIds(string table, string field)
        {
            ...
            int an = GetCount(sql_string);
            reporter.LogLine("Number of unmatched nulls in actual: " + an.ToString());
            return an;
        }
        ...
        private void CompareAttributeNumbers(string main_table, string table)
        {
            ...
            {
                reporter.BlankLine();
                reporter.LogSimpleLine("Non matching details");
                reporter.BlankLine();
        ...
        private int GetMatchingAttributeNumbers(string main_table, string table)
        {
            ...
            int m = GetCount(sql_string);
            reporter.LogLine("Number of matching attribute numbers: " + m.ToString());
            return m;
        }
        ...
        private int GetExpectedOnlyAttributeNumbers(string main_table, string table)
        {
            ...
            int e = GetCount(sql_string);
            reporter.LogLine("Numbers with attributes only in expected data: " + e.ToString());
            return e;
        }
        ...
        private int GetActualOnlyAttributeNumbers(string main_table, string table)
        {
            ...
            int a = GetCount(sql_string);
            reporter.LogLine("Numbers with attributes only in actual data: " + a.ToString());
            return a;
        }
        ...
        private int GetNumbersWhereNoAttributes(string main_table, string table)
        {
            ...
            int a = GetCount(sql_string);
            reporter.LogLine("Numbers with no attributes of this type in expected and actual: " + a.ToString());
            return a;
        }
        ...
        private void WriteOutAttributesWithNonMatchingValues(string main_table, string table)
        {
            ...
            {
                sql_string = @"select a.sd_sid as id, 
            ...
            {
                sql_string = @"select b.sd_sid as id, a.sd_oid as oid, 
                e.num as expected, a.num as actual from
            ...
            WriteOutNonMatchingAttributes(sql_string, main_table);
        }
        ...
        private void WriteOutAttributesinExpectedDataOnly(string main_table, string table)
        {
            if (main_table == "studies")
            {
                sql_string = @"select e.sd_sid as id, 
            ...
            {
                sql_string = sql_string = @"select b.sd_sid as id, e.sd_oid as oid, 
                    e.num as number from
                    (select sd_oid, count(id) as num from expected." + table + @"
                    group by sd_oid) e
                    left join
            ...
            WriteOutAttributesOnlyInOneDataset(sql_string, main_table);
        }
        ...
        private void WriteOutAttributesinActualDataOnly(string main_table, string table)
        {
            ...
            {
                sql_string = sql_string = @"select b.sd_sid as id, a.sd_oid as oid, 
                a.num as number from
                (select sd_oid, count(id) as num from expected." + table + @"
                group by sd_oid) e
                right join
        ...
        private int GetMatchingHashes(string main_table, int hashtype)
        {
            ...
            int m = GetCount(sql_string);
            reporter.LogLine("Number of matching composite hashes: " + m.ToString());
            return m;
        }
        ...
        private int GetNonMatchingHashes(string main_table, int hashtype)
        {
            ...
            int n = GetCount(sql_string);
            reporter.LogLine("Number of non-matching composite hashes: " + n.ToString());
            return n;
        }
        ...
        private int GetExpectedOnlyHashes(string main_table, int hashtype)
        {
            ...
            int e = GetCount(sql_string);
            reporter.LogLine("Numbers of composite hashes only in expected data: " + e.ToString());
            return e;
        }
        ...
        private int GetActualOnlyHashes(string main_table, int hashtype)
        {
            ...
            int a = GetCount(sql_string);
            reporter.LogLine("Numbers of composite hashes only in actual data: " + a.ToString());
            return a;
        }
        ...
        private int GetNumbersWhereNoHashes(string main_table, int hashtype)
        {
            ...
            int a = GetCount(sql_string);
            reporter.LogLine("Numbers with no composite hashes of this type in expected and actual: " + a.ToString());
            return a;
        }
        ...
        private void WriteOutHashesWithNonMatchingValues(string main_table, int hashtype)
        {
            if (main_table == "studies")
            {
                sql_string = @"select a.sd_sid as id, e.composite_hash as expected, 
            ...
            {
                sql_string = @"select b.sd_sid as id, a.sd_oid as oid, e.composite_hash as expected, 
                    a.composite_hash as actual from
            ...
            WriteOutNonMatchingHashes(sql_string, main_table);
        }
        ...
        private void WriteOutHashesinExpectedDataOnly(string main_table, int hashtype)
        {
            if (main_table == "studies")
            {
                sql_string = @"select e.sd_sid as id, e.composite_hash as hash from
                    (select sd_sid, composite_hash from expected.study_hashes
                    where hash_type_id = " + hashtype.ToString() + @") e
                    left join
                    (select sd_sid, composite_hash from adcomp.study_hashes
            ...
            {
                sql_string = @"select b.sd_sid as id, e.sd_oid as oid, e.composite_hash as hash from
                    (select sd_oid, composite_hash from expected.object_hashes
                    where hash_type_id = " + hashtype.ToString() + @") e
                    left join
                    (select sd_oid, composite_hash from adcomp.object_hashes
            ...
            WriteOutHashesOnlyInOneDataset(sql_string, main_table);
        }
        ...
        private void WriteOutHashesinActualDataOnly(string main_table, int hashtype)
        {
            if (main_table == "studies")
            {
                sql_string = @"select a.sd_sid as id, a.composite_hash as hash from
                        (select sd_sid, composite_hash from expected.study_hashes
                        where hash_type_id = " + hashtype.ToString() + @") e
                        right join
                        (select sd_sid, composite_hash from adcomp.study_hashes
            ...
            {
                sql_string = @"select b.sd_sid as id, a.sd_oid as oid, a.composite_hash as hash from
                        (select sd_oid, composite_hash from expected.object_hashes
                        where hash_type_id = " + hashtype.ToString() + @") e
                        right join
                        (select sd_oid, composite_hash from adcomp.object_hashes
        ...
        private void WriteOutNonMatchingValues(string table, string field, string fieldtype, string comptype)
        {
            ...
            sql_string = ObtainSQLForNonMatchingValues(table, field, fieldtype, comptype);
            using (var conn = new NpgsqlConnection(_db_conn))
            {
                int res_count = 0;
                if (table == "studies")
                {
                    List<DBValue> res = conn.Query<DBValue>(sql_string).ToList();
                    res_count = res.Count;
                    if (res_count > 0)
                    {
                        foreach (DBValue v in res)
                        {
                            reporter.LogSimpleLine("id: " + v.id + ", \nexpected: " + v.expected
                                              + ", \nactual: " + v.actual + "\n");
                        }
                    }
                }
                ...
                {
                    List<DBObjValue> res = conn.Query<DBObjValue>(sql_string).ToList();
                    res_count = res.Count;
                    if (res_count > 0)
                    {
                ...
                if (res_count == 0)
                {
                    reporter.LogSimpleLine("Odd - could not find the non-matching records in the DB");
                }
            }
            ...
            using (var conn = new NpgsqlConnection(_db_conn))
            {
                ...
                if (main_table == "studies")
                {
                    ...
                    if (res.Count > 0)
                    {
                        ...
                        {
                            reporter.LogSimpleLine("id: " + v.id + ", expected: " + v.expected
                                              + ", actual: " + v.actual + "\n");
                        }
                    }
                }
            ...
            using (var conn = new NpgsqlConnection(_db_conn))
            {
                ...
                if (main_table == "studies")
                {
                    ...
                    if (res_count > 0)
                    {
                        ...
                        {
                            reporter.LogSimpleLine("id: " + v.id + ", number: " + v.number + "\n");
                        }
                    }
                }
                else
            ...
            using (var conn = new NpgsqlConnection(_db_conn))
            {
                ...
                if (main_table == "studies")
                {
                    ...
                    if (res_count > 0)
                    {
                        ...
                        {
                            reporter.LogSimpleLine("id: " + v.id + ", number: " + v.hash + "\n");
                        }
                    }
                }
                else
